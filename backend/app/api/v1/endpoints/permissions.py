...
def set_role_permissions(role_id: int, data: RolePermissionUpdate, db: Session = Depends(get_db)):
	return service.set_role_permissions(db, role_id, data.permission_ids)
...
def set_user_permissions(user_id: int, data: UserPermissionUpdate, db: Session = Depends(get_db)):
	return service.set_user_permissions(db, user_id, data)
...
def get_effective_permissions(user_id: int, db: Session = Depends(get_db)):
	return service.get_effective_permissions(db, user_id)


@router.get("/me/effective", response_model=EffectivePermissionResponse)
def get_my_effective_permissions(current_user=Depends(get_current_user)):
	return service.build_effective_permission_response(current_user)
